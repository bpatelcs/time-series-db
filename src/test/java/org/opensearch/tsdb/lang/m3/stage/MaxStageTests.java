...
public class MaxStageTests extends AbstractWireSerializingTestCase<MaxStage> {
    ...
    public void setUp() throws Exception {
        super.setUp();
        maxStage = new MaxStage();
        maxStageWithLabels = new MaxStage("service");
    }

    public void testProcessWithoutGrouping() {
        // Test process() without grouping - should find max across all time series
        List<TimeSeries> input = TEST_TIME_SERIES;
        List<TimeSeries> result = maxStage.process(input);

        assertEquals(1, result.size());
        TimeSeries maxed = result.get(0);
        assertEquals(3, maxed.getSamples().size());

        // Check that maximum values are found correctly
        assertEquals(
            List.of(
                new FloatSample(1000L, 20.0), // max(10, 20, 5, 3, 1) = 20
    ...
    public void testProcessWithGrouping() {
        ...
        List<TimeSeries> input = TEST_TIME_SERIES;
        List<TimeSeries> result = maxStageWithLabels.process(input);

        assertEquals(3, result.size()); // Should have 3 groups: api, service1, and service2

        // Find the api group (ts1, ts2) - max(10,20), max(20,40), max(30,60)
        ...
        assertEquals(
            List.of(
                new FloatSample(1000L, 20.0), // max(10, 20) = 20
    ...
    public void testFromArgsNoGrouping() {
        MaxStage stage = MaxStage.fromArgs(Map.of());
        assertNotNull(stage);
        assertTrue(stage.getGroupByLabels().isEmpty());
    }

    public void testFromArgsWithSingleLabel() {
        MaxStage stage = MaxStage.fromArgs(Map.of("group_by_labels", "service"));
        assertNotNull(stage);
        assertEquals(List.of("service"), stage.getGroupByLabels());
    }

    public void testFromArgsWithMultipleLabels() {
        MaxStage stage = MaxStage.fromArgs(Map.of("group_by_labels", List.of("service", "region")));
        assertNotNull(stage);
        assertEquals(List.of("service", "region"), stage.getGroupByLabels());
    }

    public void testFromArgsInvalidType() {
        assertThrows(IllegalArgumentException.class, () -> MaxStage.fromArgs(Map.of("group_by_labels", 123)));
    }

    public void testNeedsConsolidation() {
        assertFalse(maxStage.needsMaterialization());
    }
    ...
    public void testProcessWithMissingTimestamps() {
        ...
        List<TimeSeries> result = maxStage.process(inputWithGaps);
        assertEquals(1, result.size());
        TimeSeries maxed = result.get(0);

        // Should have 4 timestamps: 0s, 10s, 20s, 30s
        assertEquals(4, maxed.getSamples().size());
        assertEquals(
            List.of(
    ...
    public void testProcessWithMissingTimestampsGrouped() {
        ...
        List<TimeSeries> result = maxStageWithLabels.process(inputWithGaps);
        assertEquals(1, result.size()); // Only service1 group
        TimeSeries maxed = result.get(0);
        assertEquals("service1", maxed.getLabels().get("service"));

        // Should have 4 timestamps: 0s, 10s, 20s, 30s with maximum values
        assertEquals(4, maxed.getSamples().size());
        assertEquals(
            List.of(
    ...
    public void testGetName() {
        assertEquals("max", maxStage.getName());
    }

    public void testProcessEmptyInput() {
        List<TimeSeries> result = maxStage.process(List.of());
